...
const AttendanceTrendChart = dynamic(
  () => import("./components/attendance-trend-chart"),
  {
    ssr: false,
    loading: () => (
...
const App = () => {
  ...
  const [deletingSubjectId, setDeletingSubjectId] = useState<string | null>(
    null
  ); // Added for delete loading state
  ...
  const calculateCumulativeAttendance = () => {
    ...
    const totalHappened = subjects.reduce(
      (sum, subject) => sum + subject.happened,
      0
    );

    const currentPercentage =
      totalHappened > 0
        ? ((totalAttended / totalHappened) * 100).toFixed(1)
        : 0;
    const requiredClasses =
      totalHappened > 0
        ? Math.ceil((0.75 * totalHappened - totalAttended) / 0.25)
        : 0;

    // Calculate how many classes can be missed while maintaining 75%
    const classesCanMiss =
      totalHappened > 0 ? Math.floor(totalAttended - 0.75 * totalHappened) : 0;
    ...
    async (id, updatedData) => {
      ...
      try {
        ...
        if (
          ...
          updatedData.happened !== undefined
        ) {
          ...
          if (
            updatedData.attended > currentSubject.attended &&
            updatedData.happened > currentSubject.happened
          ) {
            // Student attended class
            await addAttendanceHistory(
              user,
              id,
              currentSubject.name,
              true,
            ...
            if (user && id) {
              ...
              try {
                ...
                const q = query(
                  ...
                  where("subjectId", "==", id), // Make sure to import `where` from "firebase/firestore"
                  orderBy("createdAt", "desc"), // Assuming 'createdAt' is the timestamp to order by. Or 'date'.
                  limit(1)
                );
                ...
                if (!querySnapshot.empty) {
                  ...
                  console.log(
                    "Successfully deleted last attendance entry for undo:",
                    lastEntryDoc.id
                  );
                } else {
                  console.log(
                    "No attendance history found for subject to delete on undo:",
                    id
                  );
                }
              } catch (error) {
                console.error(
                  "Error during undo operation for attendance history:",
                  error
                );
              }
            }
  ...
  const handleDeleteSubject = async (subjectId: string) => {
    ...
    try {
      ...
      await deleteUserDocument(user, "marks", subjectId); // Assuming subjectId is used for marks doc ID
      ...
      toast.error(
        "Failed to delete subject and associated data. Please try again."
      );
    } finally {
  ...
  const calculateNeededClasses = useCallback((subject) => {
    ...
    const numerator =
      (subject.happened * goalPercentage) / 100 - subject.attended;
    ...
    <ProtectedRoute>
      <div className="w-[400px] md:w-[768px] lg:w-[1024px] p-4 md:p-6 lg:p-8 pb-20 md:pb-8 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
        ...
        <main className="space-y-6">
          ...
          <Sheet
            open={showAttendanceTrend}
            onOpenChange={setShowAttendanceTrend}
          >
            <SheetContent
              side="bottom"
                  ...
                  <div className="space-y-3">
                    {Number(currentPercentage) >= 75 ? (
                      <div className="flex items-center text-green-600 dark:text-green-400">
                        ...
                        <span>
                          Your overall attendance is above the required 75%
                          threshold.
                        </span>
                      </div>
                    ) : (
                    ...
                    {subjects.filter(
                      ...
                      <div className="flex items-center text-red-600 dark:text-red-400">
                        ...
                        <span>
                          ...
                          subjects have attendance below 75%.
                        </span>
                      </div>
                    )}
                    ...
                    {Number(currentPercentage) < 75 && (
                      <div className="bg-primary-100 dark:bg-primary-900 p-3 rounded-lg mt-4">
                        ...
                        <p className="text-sm">
                          To reach 75% attendance, you need to attend{" "}
                          {requiredClasses} more classes without missing any.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <SheetContent>
                ...
                <form onSubmit={addSubject} className="space-y-4 mt-4">
                  ...
                  <Input
                    ...
                    value={newAttended}
                    onChange={(e) =>
                      setNewAttended(Number.parseInt(e.target.value, 10))
                    }
                    required
                  />
                  <Input
                    ...
                    value={newHappened}
                    onChange={(e) =>
                      setNewHappened(Number.parseInt(e.target.value, 10))
                    }
                    required
                  />
            ...
            >
              ...
              <div className="p-4 pt-0 space-y-4 border-t border-gray-100 dark:border-gray-700">
                {subjects.map((subject) => {
                  const currentPercentage =
                    subject.happened > 0
                      ? ((subject.attended / subject.happened) * 100).toFixed(1)
                      : 0;
                    ...
                    <div key={subject.id} className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {subject.attended}/{subject.happened} (
                          {currentPercentage}%)
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        Goal: {goalPercentage}% • Classes Needed:{" "}
                        {neededClasses}
                      </div>
                    </div>
                  );
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    {subject.name}" and all its associated data,
                                    including attendance and marks history.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel
