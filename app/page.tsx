...
const App = () => {
  ...
  const addSubject = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault()
      if (!newSubject || newAttended < 0 || newHappened < 0) return
    ...
    [newSubject, newAttended, newHappened],
  )

  const updateSubject = useCallback(async (id, updatedData) => {
    try {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const totalAchieved = marksData.reduce((sum, subject) => sum + subject.totalScoredMarks, 0)
      const totalTaken = marksData.reduce((sum, subject) => sum + subject.totalMaxMarks, 0)
      const totalPossible = marksData.reduce((sum, subject) => sum + subject.credits * 25, 0)
      ...
      const weightedGPA = marksData.reduce((sum, subject) => sum + subject.gradePoint * subject.credits, 0)
      const totalPercentage = marksData.reduce((sum, subject) => sum + subject.percentage, 0)
      const avgGPA = totalCredits > 0 ? weightedGPA / totalCredits : 0
      const avgPercentage = marksData.length > 0 ? totalPercentage / marksData.length : 0

      // Generate the PDF
      await generatePDFReport(
        subjects,
        marksData,
        { currentPercentage, requiredClasses },
        { totalAchieved, totalTaken, totalPossible, overallPercentage, progressPercentage },
        { avgPercentage, avgGPA },
      )
    } catch (error) {
    ...
    <div className="w-[400px] md:w-[768px] lg:w-[1024px] p-4 md:p-6 lg:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
      ...
      <main className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {Number(currentPercentage) < 75 && (
                    <div className="bg-primary-100 dark:bg-primary-900 p-3 rounded-lg mt-4">
                      ...
                      <p className="text-sm">
                        To reach 75% attendance, you need to attend {requiredClasses} more classes without missing any.
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <Link href="/marks-trend">
            <div className="flex flex-col items-center justify-center bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm transition-all duration-200 hover:shadow-md hover:bg-gray-50 dark:hover:bg-gray-700 hover:-translate-y-1 group">
              <span className="material-symbols-outlined text-2xl text-primary-500 mb-2 group-hover:scale-110 transition-transform">
                trending_up
              </span>
              <span className="text-sm font-medium">Marks Trend</span>
            </div>
          </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {subjects.map((subject) => {
            ...
            return (
              ...
              >
                <div className={`h-28 bg-gradient-to-r ${gradientColor} p-4 flex flex-col justify-between text-white`}>
                          ...
                          <div className="space-y-2">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                updateSubject(subject.id, { customGoalPercentage: value })
                              }}
