...
export const groupAttendanceByPeriod = (
  ...
  attendanceHistory.forEach((entry) => {
    ...
    // Count all classes that happened (both attended and missed)
    if (entry.happened) {
      group.totalClasses += 1

      // Only count as attended if the student actually attended
      if (entry.attended) {
    ...
    // Count all classes for this subject
    if (entry.happened) {
      subjectData.totalClasses += 1

      // Only count as attended if actually attended
      if (entry.attended) {
  ...
  return Array.from(grouped.values())
    .map((group) => ({
      ...group,
      percentage: group.totalClasses > 0 ? (group.attendedClasses / group.totalClasses) * 100 : 0,
      ...
      subjectsData: Array.from(group.subjects.entries()).map(([name, data]) => ({
        ...
        percentage: data.totalClasses > 0 ? (data.attendedClasses / data.totalClasses) * 100 : 0,
      })),
    }))
