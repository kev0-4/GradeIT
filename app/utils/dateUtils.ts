...
export const groupAttendanceByPeriod = (
  ...
  const result = Array.from(grouped.values())
    .map((group) => ({
      ...group,
      percentage:
        group.totalClasses > 0
          ? Math.round(
              (group.attendedClasses / group.totalClasses) * 100 * 100
            ) / 100
          : 0,
      subjectCount: group.subjects.size,
      subjectsData: Array.from(group.subjects.entries()).map(
        ([name, data]) => ({
          name,
          ...data,
          percentage:
            data.totalClasses > 0
              ? Math.round(
                  (data.attendedClasses / data.totalClasses) * 100 * 100
                ) / 100
              : 0,
        })
      ),
...
export const groupMarksByPeriod = (
  marksHistory: any[],
  period: "daily" | "weekly" | "monthly" = "monthly"
) => {
  const grouped = new Map();
  ...
  return Array.from(grouped.values())
    .map((group) => ({
      ...group,
      percentage:
        group.totalMarks > 0 ? (group.scoredMarks / group.totalMarks) * 100 : 0,
      subjectCount: group.subjects.size,
      subjectsData: Array.from(group.subjects.entries()).map(
        ([name, data]) => ({
          name,
...
export const getSubjectWiseAttendance = (subjects: any[]) => {
  return subjects.map((subject) => ({
    ...
    percentage:
      (subject.happened || 0) > 0
        ? ((subject.attended || 0) / subject.happened) * 100
        : 0,
