...
import { Button as ShadButton } from "@/components/ui/button"; // Renamed to avoid conflict if original Button is used differently
import { toast } from "sonner";
// End imports for delete functionality

// Add imports for authentication
import ProtectedRoute from "../components/protected-route";
import UserProfile from "../components/user-profile";
import { useAuth } from "../contexts/auth-context"; // Already here, good.
import {
  getUserDocuments,
  addUserDocument,
  updateUserDocument,
  addMarksHistory,
  deleteUserDocument, // Added
  deleteAttendanceHistoryForSubject, // Added
  deleteMarksHistoryForSubject, // Added
} from "../utils/user-data"; // Path is correct as per instruction
...
const MarksPage = () => {
  ...
  const [editingSubject, setEditingSubject] = useState<SubjectMarks | null>(
    null
  );
  ...
  const [subjectToDelete, setSubjectToDelete] = useState<SubjectMarks | null>(
    null
  );
  ...
  const calculateMarksSummary = () => {
    const totalAchieved = subjectsMarks.reduce(
      (sum, subject) => sum + subject.totalScoredMarks,
      0
    );
    const totalTaken = subjectsMarks.reduce(
      (sum, subject) => sum + subject.totalMaxMarks,
      0
    );
    const totalPossible = subjectsMarks.reduce(
      (sum, subject) => sum + subject.credits * MARKS_PER_CREDIT,
      0
    );

    const overallPercentage =
      totalTaken > 0 ? ((totalAchieved / totalTaken) * 100).toFixed(1) : "0";
    const progressPercentage =
      totalPossible > 0
        ? ((totalAchieved / totalPossible) * 100).toFixed(1)
        : "0";
  ...
  const calculateAverages = () => {
    ...
    const totalCredits = subjectsMarks.reduce(
      (sum, subject) => sum + subject.credits,
      0
    );
    const weightedGPA = subjectsMarks.reduce(
      (sum, subject) => sum + subject.gradePoint * subject.credits,
      0
    );
    const totalPercentage = subjectsMarks.reduce(
    ...
    const avgGPA =
      totalCredits > 0
        ? Number.parseFloat((weightedGPA / totalCredits).toFixed(2))
        : 0;
    const avgPercentage =
      subjectsMarks.length > 0
        ? Number.parseFloat((totalPercentage / subjectsMarks.length).toFixed(2))
        : 0;
  ...
  const calculateMinimumMarksNeeded = (
    ...
    const remainingMarksAvailable =
      totalMaxPossibleMarks - subject.totalMaxMarks;
    ...
    if (subject.percentage >= goalPercentage) {
      ...
      const minPercentageInRemaining =
        remainingMarksAvailable > 0
          ? Math.min(
              100,
              Math.max(0, (minFutureMarks / remainingMarksAvailable) * 100)
            )
          : 0;

      // Calculate minimum marks needed per exam
      const minMarksPerExam =
        remainingExams > 0 ? Math.ceil(minFutureMarks / remainingExams) : 0;
    ...
    const minMarksPerExam =
      remainingExams > 0
        ? Math.ceil(additionalMarksNeeded / remainingExams)
        : 0;
    ...
    return subjectsMarks.map((subject) => {
      ...
      if (currentGP >= goalGPA) {
        ...
        const targetTotalScore =
          (neededPercentage / 100) * totalMaxPossibleMarks;
        const minFutureMarks = Math.max(
          0,
          targetTotalScore - subject.totalScoredMarks
        );
        const remainingMarksAvailable =
          totalMaxPossibleMarks - subject.totalMaxMarks;
      ...
      const additionalMarksNeeded = Math.max(
        0,
        targetTotalScore - subject.totalScoredMarks
      );
  ...
  const updateSubjectMarks = async (
    id: string,
    updatedData: Partial<SubjectMarks>
  ) => {
    if (!user) return;
  ...
  const addExamToSubject = async (subjectId: string) => {
    if (!newExam.examName || newExam.scoredMarks < 0 || newExam.totalMarks <= 0)
      return;
    ...
    const totalScoredMarks = updatedExams.reduce(
      (sum, exam) => sum + exam.scoredMarks,
      0
    );
    const totalMaxMarks = updatedExams.reduce(
  ...
  const deleteExam = async (subjectId: string, examIndex: number) => {
    ...
    const updatedExams = subject.exams.filter(
  ...
  const handleOpenDeleteDialog = (subject: SubjectMarks) => {
    ...
    // AlertDialog will be triggered by its own trigger within DropdownMenuItem
  };

  const handleConfirmDelete = async () => {
    if (!user || !subjectToDelete || !subjectToDelete.id) {
      toast.error(
        "User not authenticated or subject data missing. Cannot delete."
      );
      return;
    }
    ...
    try {
      ...
      await deleteUserDocument(user, "subjects", subjectToDelete.id); // Also delete from subjects collection

      setSubjectsMarks((prev) =>
        prev.filter((s) => s.id !== subjectToDelete.id)
      );
      toast.success(
        `Subject "${subjectToDelete.subjectName}" and all associated data deleted.`
      );
    } catch (error) {
      console.error("Error deleting subject:", error);
      toast.error(
        `Failed to delete subject "${subjectToDelete.subjectName}". Please try again.`
      );
    } finally {
  ...
  return (
    <ProtectedRoute>
      <div className="w-[400px] md:w-[768px] lg:w-[1024px] p-4 md:p-6 lg:p-8 pb-20 md:pb-8 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
        ...
        <main className="space-y-6">
              ...
              <SheetContent>
                ...
                <form
                  onSubmit={(e) => {
                    ...
                    const maxExams = Number.parseInt(
                      formData.get("maxExams") as string,
                      10
                    );
                ...
                <div className="mt-4 p-3 bg-primary-100 dark:bg-primary-900 rounded-lg">
                  ...
                  <p className="text-xs mt-1">
                    {avgGPA >= 8 ? (
                      ...
                      <>
                        To reach a GPA of 8.0, focus on improving performance in
                        subjects with lower grades, particularly those with more
                        remaining exams and higher credit values.
                      </>
                    )}
                  </p>
                </div>
          ...
          <Sheet open={showVisualizations} onOpenChange={setShowVisualizations}>
            <SheetContent
              side="bottom"
              className="h-[80vh] p-0 overflow-hidden flex flex-col"
            >
                    ...
                    {subjectsMarks
                      ...
                      .map((subject) => (
                        <div key={subject.id} className="flex items-center">
                          <div
                            className="w-24 truncate text-xs mr-2"
                ...
                <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-4 mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {subjectsMarks.map((subject) => {
                      ...
                      const lost =
                        subject.totalMaxMarks - subject.totalScoredMarks;
                      ...
                      return (
                        <div key={subject.id} className="flex-1 min-w-[150px]">
                          ...
                          <div className="text-[10px] text-gray-500 dark:text-gray-400">
                            {scored}/{totalPossible} marks (
                            {Math.round((scored / totalPossible) * 100)}%)
                          </div>
                        </div>
                      );
                    })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {subjectsMarks.map((subject) => {
              ...
              const remainingExams = subject.maxExams - subject.exams.length;
              const { minPercentage, minMarks, marksPerExam, achievable } =
                calculateMinimumMarksNeeded(subject, goalPercentage);
              const totalMaxPossibleMarks = subject.credits * MARKS_PER_CREDIT;
              ...
              return (
                ...
                >
                  <div
                    className={`h-28 bg-gradient-to-r ${gradientColor} p-4 flex flex-col justify-between text-white relative`} // Added relative positioning
                  >
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold mr-2 truncate">
                        {subject.subjectName}
                      </h3>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <ShadButton
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 text-white hover:bg-white/20 focus-visible:ring-0"
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() => setEditingSubject(subject)} // Assuming you have an edit feature
                            >
                            ...
                            <DropdownMenuSeparator />
                            {/* AlertDialogTrigger removed from here */}
                            <DropdownMenuItem
                              className="text-red-600 dark:text-red-500 hover:!text-red-600 dark:hover:!text-red-500 focus:text-red-600 dark:focus:text-red-500 cursor-pointer"
                              onSelect={() => handleOpenDeleteDialog(subject)} // Changed to onSelect for DropdownMenuItem, more idiomatic
                              disabled={deletingSubjectId === subject.id}
                            >
                              <span className="material-symbols-outlined text-sm mr-2">
                                delete
                    ...
                    <div className="flex justify-between mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xl font-bold text-primary-600 dark:text-primary-400">
                          {Math.floor(
                            (subject.totalScoredMarks / totalMaxPossibleMarks) *
                              100
                          )}
                          %
                        </p>
                          ...
                          <div className="space-y-2">
                            <div className="grid grid-cols-3 gap-2">
                              ...
                              <Button
                                ...
                                disabled={
                                  !newExam.examName || newExam.totalMarks <= 0
                                }
            ...
            <Sheet>
              <SheetTrigger asChild>
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm text-center cursor-pointer hover:shadow-md transition-shadow">
                  <span className="material-symbols-outlined text-4xl text-primary-500 mb-2">
                    add_circle
                  </span>
      ...
      {/* Alert Dialog for Delete Confirmation - Placed outside main layout div, controlled by state */}
      <AlertDialog
        open={!!subjectToDelete}
        onOpenChange={(open) => {
          if (!open) {
            setSubjectToDelete(null); // Close dialog if open is false
          }
        }}
      >
        {subjectToDelete && ( // Ensure content is only rendered when there's a subject to delete
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the
                subject "<b>{subjectToDelete.subjectName}</b>" and all its
                associated data, including marks history and attendance records.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
