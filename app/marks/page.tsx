...
const MarksPage = () => {
  ...
  // Calculate marks needed to reach goal GPA
  const calculateMarksNeeded = (goalGPA = 8) => {
    return subjectsMarks.map((subject) => {
      ...
      // If no remaining exams, goal can't be achieved
      if (remainingExams <= 0) {
        return {
          ...subject,
          neededPercentage,
      ...
      return {
        ...subject,
        neededPercentage,
        additionalMarksNeeded: Math.max(0, Math.ceil(additionalMarksNeeded)),
  ...
  const addSubjectMarks = async (subject: SubjectMarks) => {
    try {
      const docRef = await addDoc(collection(db, "marks"), subject)
  ...
  const updateSubjectMarks = async (id: string, updatedData: Partial<SubjectMarks>) => {
    try {
      await updateDoc(doc(db, "marks", id), updatedData)
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const attendanceSnapshot = await getDocs(collection(db, "subjects"))
      ...
      const totalHappened = attendanceData.reduce((sum, subject) => sum + subject.happened, 0)
              ...
              {marksNeededData.map((subject) => (
                <div key={subject.id} className="border-b border-gray-100 dark:border-gray-700 pb-3 last:border-0">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                    ...
                    <div>
                      <p className="text-gray-500 dark:text-gray-400">Min % in Remaining Exams</p>
                      <p
                        className={
                          subject.gradePoint >= 8
                            ? "text-green-600 dark:text-green-400"
                      ...
                      >
                        {subject.gradePoint >= 8 ? "0%" : `${subject.minPercentageInRemaining}%`}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 p-3 bg-primary-100 dark:bg-primary-900 rounded-lg">
                ...
                <p className="text-xs mt-1">
                  {avgGPA >= 8 ? (
                    ...
                    <>
                      To reach a GPA of 8.0, focus on improving performance in subjects with lower grades, particularly
                      those with more remaining exams and higher credit values.
                    </>
                  )}
                </p>
              </div>
